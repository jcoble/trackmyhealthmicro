...
$do$
    BEGIN
        IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'track_my_health') THEN
            CREATE DATABASE track_my_health;
        END IF;
    END
$do$;
...
$DO$
    ...
    BEGIN
        IF NOT EXISTS(
            ...
            CREATE USER root SUPERUSER CREATEDB CREATEROLE REPLICATION BYPASSRLS;
        END
            IF;
    END
$DO$;
...
CREATE TABLE public.app_user (
                                 app_user_auth_id uuid NOT NULL DEFAULT uuid_generate_v1(),
                                 profile_url text,
                                 last_login_ip text,
                                 ...
                                 ref_id_str text,
                                 meta text,  -- <====
                                 CONSTRAINT app_user_auth_id_key UNIQUE (app_user_auth_id)
);
...
--
-- create database if not exists track_my_health; --username=root --owner=root track_my_health
