...
<div>
    ...
    <input
            class="{cls} input input-bordered border-primary-focus input-secondary w-full max-w-xs"
            {type}
            {name}
            {value}
            {placeholder}
    ...
    />
    {#if errorField}
        <div class="mb-2 text-error-content invalid-feedback mt-0 pt-0">{errorField}</div>
    {/if}
</div>
