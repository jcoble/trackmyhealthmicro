<script lang="ts">
    ...
    import {userSession, client, redirect, checkAuth} from '$shared';
    import register from '$root/routes/auth/register/index.svelte'
    import {Hello, Authenticate} from '$elements/dtos';
    ...
    const submit = async () => {
        try {
            ...
            const response = await client.post(new Authenticate({
                provider: 'credentials', email, password, rememberMe,
                provider: 'credentials', email, password, rememberMe,
            }));
...
<div class="bg-opacity-20 relative z-10 h-auto p-8 py-10 overflow-hidden border-gray-300 shadow-2xl px-7 Login rounded-lg">
    ...
    <div class="block">
        <ButtonGlow loading=false></ButtonGlow>
    </div>
